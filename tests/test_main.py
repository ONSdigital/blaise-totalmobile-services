...
class TestCreateTotalmobileJobsTrigger:
    @mock.patch(
        "factories.service_instance_factory.ServiceInstanceFactory.create_totalmobile_jobs_service"
    )
    @mock.patch(
    ...
    def test_create_totalmobile_jobs_trigger_logs_an_error_and_throws_an_exception_when_request_json_is_empty(
        ...
        assert (
            "Could not parse JSON request: 'NoneType' object has no attribute 'get_json'"
            in caplog.messages
        )
...
class TestCreateTotalmobileJobsTriggerLMS:
    mock_request = flask.Request.from_values(json={"survey_type": "LMS"})
    ...
    def test_create_totalmobile_jobs_trigger_is_called_the_correct_number_of_times_with_the_correct_information(
        self, mock_create_totalmobile_jobs_trigger, mock_create_totalmobile_jobs_service
    ):
        # arrange
        mock_totalmobile_jobs_service_instance = (
            mock_create_totalmobile_jobs_service.return_value
        )
...
class TestCreateTotalmobileJobsTriggerFRS:
    mock_request = flask.Request.from_values(json={"survey_type": "FRS"})
