Feature: Totalmobile update
    Scenario: Match a totalmobile update with data in blaise
        Given there is a questionnaire "LMS2206-AA1" with case "12345" in Blaise
    ...
    Scenario: Reference is not matched with a questionnaire in Blaise
        Given there is no questionnaire "LMS2206-AA1" in Blaise
        When Totalmobile sends an update for reference "LMS2206-AA1.12345"
    ...
    Scenario: Reference is not matched with a case in Blaise
        Given there is a questionnaire "LMS2206-AA1" in Blaise
        But there is no case "12345" for questionnaire "LMS2206-AA1" in Blaise 
        When Totalmobile sends an update for reference "LMS2206-AA1.12345"
        Then "Could not find case 12345 for questionnaire LMS2206-AA1 in Blaise" is logged as an error message
        And a "404 Not Found" response is sent back to Totalmobile
