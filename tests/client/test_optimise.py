...
def test_create_job(requests_mock, optimise_client, mock_auth_response):
    ...
    assert optimise_client.create_job("test", {}) == {}


def test_create_job_failing_auth_403(requests_mock, optimise_client, mock_auth_response):
    requests_mock.post("/Test/identity/connect/token", json=mock_auth_response)
    requests_mock.post(
        "/Test/api/optimise/worlds/test/jobs", status_code=403
    )
    with pytest.raises(AuthException):
        optimise_client.create_job("test", {})


def test_create_job_failing_auth_401(requests_mock, optimise_client, mock_auth_response):
    requests_mock.post("/Test/identity/connect/token", json=mock_auth_response)
    requests_mock.post(
        "/Test/api/optimise/worlds/test/jobs", status_code=401
    )
