...
class DeleteTotalmobileJobsService:
    ...
    def delete_totalmobile_jobs_completed_in_blaise(self) -> None:
        ...
        for questionnaire_name in totalmobile_jobs_model.questionnaire_jobs.keys():
            if self.questionnaire_has_incomplete_cases(questionnaire_name, totalmobile_jobs_model):
                continue
    ...
    @staticmethod
    def questionnaire_has_incomplete_cases(questionnaire_name: str, totalmobile_jobs_model: TotalmobileJobsResponseModel) -> bool:
        return all(job.visit_complete for job in totalmobile_jobs_model.questionnaire_jobs[questionnaire_name])
