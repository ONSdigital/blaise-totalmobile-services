...
def update_case(
    ...
    validate_questionnaire_exists(
        totalmobile_request.questionnaire_name, config, questionnaire_service
    )

    blaise_case = get_case(
    ...
    if totalmobile_request.outcome_code == 300 and blaise_case.outcome_code in (
        ...
        _update_case_contact_information(
            blaise_case, update_blaise_case_model, config, questionnaire_service
        )
        return

    if totalmobile_request.outcome_code in (460, 461, 510, 540, 551, 560, 580, 640):
        _update_case_outcome_code(
            blaise_case, update_blaise_case_model, config, questionnaire_service
        )
        return
...
def _update_case_contact_information(
    ...
    questionnaire_service.update_case(
        blaise_case.questionnaire_name, blaise_case.case_id, fields_to_update, config
    )
...
def validate_questionnaire_exists(
    questionnaire_name: str, config: Config, questionnaire_service
) -> None:
...
def get_case(
    questionnaire_name: str, case_id: str, config: Config, questionnaire_service
